  ...
  <div id="detail">
    ...
    <section class="article">
      ...
      <p>
        ...
        由于这里所涉及的术语在概念上都是相似的，如果不先记住它们在网格规范中定义的含义，则很容易将它们彼此混淆。
        但是不用太担心，这些术语并不多。
      </p>
      <h3 id="grid-container">Grid Container</h3>
      <p>
        设置了 <code>display: gird</code> 的元素。 这是所有 grid item
        的直接父项。 在下面的例子中，<code>.container</code> 就是是 grid
        container。
      </p>
...
<span class="hljs-tag">&lt;/<span class="hljs-name">div</span>&gt;</span>
      ...
      <p>
        Grid 容器的孩子（直接子元素）。下面的 <code>.item</code> 元素就是 grid
        item，但 <code>.sub-item</code>不是。
      </p>
      <pre><code class="lang-html"><span class="hljs-tag">&lt;<span class="hljs-name">div</span> <span class="hljs-attr">class</span>=<span class="hljs-string">"container"</span>&gt;</span>
...
<span class="hljs-tag">&lt;/<span class="hljs-name">div</span>&gt;</span>
      ...
      <p>
        这个分界线组成网格结构。 它们既可以是垂直的（“column grid
        lines”），也可以是水平的（“row grid lines”），并位于行或列的任一侧。
        下面例中的黄线就是一个列网格线。
      </p>
      <p>
        <img
      ...
      <p>
        将元素定义为 grid contaienr，并为其内容建立新的网格格式化上下文(grid
        formatting context)。
      </p>
      <p>值:</p>
      <ul>
        ...
        <li>
          <strong>subgrid</strong> - 如果你的 grid container 本身就是一个 grid
  ...
  <span class="hljs-attribute">display</span>: grid | inline-grid | subgrid;
      ...
      <p>
        注意：<code>column</code>, <code>float</code>, <code>clear</code>, 以及
        <code>vertical-align</code> 对一个 grid container 没有影响
      </p>
      <h3 id="grid-template-columns--grid-template-rows">
        grid-template-columns / grid-template-rows
      </h3>
      <p>
        使用以空格分隔的多个值来定义网格的列和行。这些值表示轨道大小(track
        size)，它们之间的空格代表表格线(grid line)。
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-rows</span>: [row1-start] <span class="hljs-number">25%</span> [row1-end] <span class="hljs-number">100px</span> [third-line] auto [last-line];
      ...
      <p>
        需要注意的是，一个网格线可以有不止一个名字。例如，这里第2条网格线有两个名字：row1-end
        和 row2-start：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  <span class="hljs-attribute">grid-template-rows</span>: [row1-start] <span class="hljs-number">25%</span> [row1-end row2-start] <span class="hljs-number">25%</span> [row2-end];
      ...
      <p>
        如果你的定义中包含重复的部分，则可以使用<code>repeat()</code>
        符号来简化写法：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-columns</span>: <span class="hljs-number">20px</span> [col-start] <span class="hljs-number">20px</span> [col-start] <span class="hljs-number">20px</span> [col-start] <span class="hljs-number">5%</span>;
      ...
      <p>
        ...
        例如，下面的代码会将每个 grid item 为 grid container 宽度的三分之一：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  <span class="hljs-attribute">grid-template-columns</span>: <span class="hljs-number">1</span>fr <span class="hljs-number">1</span>fr <span class="hljs-number">1</span>fr;
      ...
      <p>
        自由空间是在排除所有不可伸缩的 grid item 之后计算得到的。
        在下面的示例中，fr单位可用的自由空间总量不包括50px:
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  <span class="hljs-attribute">grid-template-columns</span>: <span class="hljs-number">1</span>fr <span class="hljs-number">50px</span> <span class="hljs-number">1</span>fr <span class="hljs-number">1</span>fr;
      ...
      <p>
        通过引用 <code>grid-area</code>属性指定的网格区域的名称来定义网格模板。
      ...
      <ul>
        <li>
          <code>&lt;grid-area-name&gt;</code> - 使用 grid-area
          属性设置的网格区域的名称
        </li>
...
<span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-areas</span>: 
    ...
    <span class="hljs-string">"footer footer footer footer"</span>;
      ...
      <p>
        ...
        <strong>sidebar</strong> 区域组成。
        最后一行是<strong>footer</strong>区域组成。
      </p>
      <p>
        <img
      ...
      <p>
        您可以使用任意数量的相邻的<code>.</code>来声明单个空单元格。
        只要这些点号之间没有空格，他们就代表了一个单一的单元格。
      </p>
      <p>
        需要注意的是你不是在用这个语法命名网格线，而是在命名区域。
        当你使用这种语法时，区域两端的网格线实际上是自动命名的。
        比如，如果网格区域的名称是foo，那么区域的起始的行网格线和列网格线名称是
        foo-<strong>start</strong>，并且区域终点的行网格线和列网格线名称是
        foo-<strong>end</strong>。
        这意味着某些网格线可能有多个名称，比如上面的例子中最左边的一条网格线有三个名字：header-start，main-start
        和 footer-start。
      </p>
      ...
      <ul>
        ...
        <li>
          <strong>subgrid</strong> - 把 grid-template-rows 和
          grid-template-columns 设置为 subgrid, 并且 grid-template-areas
          设置为初始值
        </li>
        <li>
          <code>grid-template-rows</code> /
          <code>&lt;grid-template-columns</code> - 把 grid-template-columns 和
          grid-template-rows 设置为指定值, 与此同时, 设置 grid-template-areas 为
          none
        </li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-gap</span>: <span class="hljs-number">10px</span> <span class="hljs-number">15px</span>;
      ...
      <p>
        沿着行轴对齐网格内的内容（与之对应的是 align-items,
  ...
  <span class="hljs-attribute">justify-items</span>: stretch;
      ...
      <p>
        也可以通过给单个 grid item
        设置<code>justify-self</code>属性来达到上述效果。
      </p>
      <h3 id="align-items">align-items</h3>
      <p>
        沿着列轴对齐grid item 里的内容（与之对应的是使用 justify-items
  ...
  <span class="hljs-attribute">align-items</span>: stretch;
      ...
      <p>
        也可以通过给单个 grid item
        设置<code>align-self</code>属性来达到上述效果。
      </p>
      <h3 id="justify-content">justify-content</h3>
      <p>
        有时，网格的总大小可能小于其网格容器的大小。如果你的所有 grid items
        都使用像px这样的非弹性单位来设置大小，则可能发生这种情况。此时，你可以设置网格容器内的网格的对齐方式。
        此属性沿着行轴对齐网格（与之对应的是 align-content, 沿着列轴对齐）。
      </p>
      <p>值：</p>
      <ul>
        ...
        <li>
          space-around - 在 grid item
          之间设置均等宽度的空白间隙，其外边缘间隙大小为中间空白间隙宽度的一半
        </li>
        <li>
  ...
  <span class="hljs-attribute">justify-content</span>: space-evenly;  
      ...
      <p>
        有时，网格的总大小可能小于其网格容器的大小。如果你的所有 grid items
        都使用像px这样的非弹性单位来设置大小，则可能发生这种情况。此时，你可以设置网格容器内的网格的对齐方式。
        此属性沿着列轴对齐网格（与之对应的是 justify-content, 即沿着行轴对齐）。
      </p>
      <p>值：</p>
      <ul>
  ...
  <span class="hljs-attribute">align-content</span>: space-evenly;  
      ...
      <h3 id="grid-auto-columns--grid-auto-rows">
        grid-auto-columns / grid-auto-rows
      </h3>
      <p>
        指定自动生成的网格轨道（又名隐式网格轨道）的大小。
        隐式网格轨道在你显式的定位超出指定网格范围的行或列（使用
        grid-template-rows/grid-template-columns）时被创建。
      </p>
      <p>值：</p>
      <ul>
        <li>
          <code>&lt;track-size&gt;</code> -
          可以是一个长度值，一个百分比值，或者一个自由空间的一部分（使用 fr
          单位）
        </li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-rows</span>: <span class="hljs-number">90px</span> <span class="hljs-number">90px</span>
      ...
      <p>这里创建了一个 2x2的网格。</p>
      <p>
        但是，现在想象一下，使用 grid-column 和 grid-row
        来定位你的网格项目，如下所示：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.item-a</span> {
...
<span class="hljs-selector-class">.item-b</span> {
  ...
  <span class="hljs-attribute">grid-row</span>: <span class="hljs-number">2</span> / <span class="hljs-number">3</span>;
      ...
      <p>
        这里我们指定 .item-b开始于列网格线 5 并结束于在列网格线
        6，但我们并未定义列网格线 5 或
        6。因为我们引用不存在的网格线，宽度为0的隐式轨道的就会被创建用与填补间隙。我们可以使用
        grid-auto-columns 和 grid-auto-rows属性来指定这些隐式轨道的宽度：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  <span class="hljs-attribute">grid-auto-columns</span>: <span class="hljs-number">60px</span>;
      ...
      <p>
        如果你存在没有显示指明放置在网格上的 grid
        item，则自动放置算法会自动放置这些项目。
...
<span class="hljs-tag">&lt;/<span class="hljs-name">section</span>&gt;</span>
      ...
      <p>
        你定义一个有5列和2行的网格，并将 grid-auto-flow 设置为
        row（这也是默认值）：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
...
<span class="hljs-selector-class">.item-e</span> {
  ...
  <span class="hljs-attribute">grid-row</span>: <span class="hljs-number">1</span> / <span class="hljs-number">3</span>;
      ...
      <p>
        因为我们将 grid-auto-flow 设置为row，所以我们的grid就像这样。
        注意观察我们没有做设置的三个项目（item-b，item-c和item-d）是如何在剩余的行水平摆放位置的：
      </p>
      <p>
        <img
      ...
      <p>
        如果我们将 grid-auto-flow 设置为 column，则 item-b，item-c 和 item-d
        以列的顺序上下摆放：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  <span class="hljs-attribute">display</span>: grid;
  ...
  <span class="hljs-attribute">grid-auto-flow</span>: column;
      ...
      <p>
        ...
        它同时也将 sets grid-column-gap 和 grid-row-gap
        设置为它们的初始值，即使它们不能被此属性显示设置。
      </p>
      <p>值：</p>
      <ul>
        ...
        <li>
          <code>&lt;grid-template-rows&gt;</code> /
          <code>&lt;grid-template-columns&gt;</code> - 将 grid-template-rows 和
          grid-template-columns 分别设置为指定值，将所有其他子属性设置为其初始值
        </li>
        <li>
          <code>&lt;grid-auto-flow&gt;</code> [<code
            >&lt;grid-auto-rows&gt;</code
          >
...
<span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-auto-columns</span>: auto;
      ...
      <p>
        它也可用使用一个更复杂但相当方便的语法来一次设置所有内容。 你可以指定
        grid-template-areas、grid-template-rows 以及
        grid-template-columns，并将所有其他子属性设置为其初始值。
        你现在所做的是在其网格区域内，指定网格线名称和内联轨道大小。
        可以看下面的例子：
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-columns</span>: auto <span class="hljs-number">50px</span> auto;    
      ...
      <h3 id="grid-column-start--grid-column-end--grid-row-start-grid-row-end">
        grid-column-start / grid-column-end / grid-row-start /grid-row-end
      </h3>
      <p>
        使用特定的网格线确定 grid item
        在网格内的位置。grid-column-start/grid-row-start 属性表示grid
        item的网格线的起始位置，grid-column-end/grid-row-end属性表示网格项的网格线的终止位置。
      </p>
      <p>值：</p>
      <ul>
        <li>
  ...
  grid-row-end: span <span class="hljs-number">2</span>
      ...
      <p>
        ...
        grid-row-end，默认情况下，该网格项将跨越1个轨道。
      </p>
      ...
      <ul>
        <li>
          <code>&lt;start-line&gt;</code> / <code>&lt;end-line&gt;</code> -
          每个值的用法都和属性分开写时的用法一样
        </li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.item</span> {
  ...
  <span class="hljs-attribute">grid-row</span>: third-line / <span class="hljs-number">4</span>;
      ...
      <h3 id="grid-area">grid-area</h3>
      <p>
        给 grid item 进行命名以便于使用 grid-template-areas
        属性创建模板时来进行引用。另外也可以做为 grid-row-start +
        grid-column-start + grid-row-end + grid-column-end 的简写形式。
      </p>
      <p>值：</p>
      <ul>
        <li><code>&lt;name&gt;</code> - 你的命名</li>
        <li>
          <code>&lt;row-start&gt;</code> / <code>&lt;column-start&gt;</code> /
          <code>&lt;row-end&gt;</code> / <code>&lt;column-end&gt;</code> -
          可以是数字，也可以是网格线的名字
        </li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.item</span> {
  ...
  <span class="hljs-attribute">grid-area</span>: header
      ...
      <p>
        作为 grid-row-start + grid-column-start + grid-row-end + grid-column-end
        的简写:
      </p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.item-d</span> {
  <span class="hljs-attribute">grid-area</span>: <span class="hljs-number">1</span> / col4-start / last-line / <span class="hljs-number">6</span>
      ...
      <p>
        沿着行轴对齐grid item 里的内容（与之对应的是 align-self,
  ...
  <span class="hljs-attribute">justify-self</span>: stretch;
      ...
      <p>
        要为网格中的所有grid items 设置对齐方式，也可以通过 justify-items
        属性在网格容器上设置此行为。
      </p>
      <h3 id="align-self">align-self</h3>
      <p>
        沿着列轴对齐grid item 里的内容（与之对应的是 justify-self,
        即沿行轴对齐）。 此属性对单个网格项内的内容生效。
      </p>
      <p>值：</p>
      <ul>
  ...
  <span class="hljs-attribute">align-self</span>: stretch;
      ...
      <p>
        要为网格中的所有grid items 统一设置对齐方式，也可以通过 align-items
        属性在网格容器上设置此行为。
      </p>
    </section>
  </div>
